...
import { Incident } from "../../convex/incidents";
...
export async function announceNewIncident(
    ...
    try {
        console.log("📢 Announcing new incident:", incident._id);
...
export async function testIncidentAnnouncement(): Promise<void> {
    try {
        ...
        const incidents = await convex.query(api.incidents.getIncidents);
        ...
        if (!latestIncident) {
            console.warn("No incidents found in database to announce");
            return;
        }

        console.log("✅ Found latest incident:", {
            id: latestIncident._id,
